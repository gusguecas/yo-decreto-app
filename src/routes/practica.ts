...
practicaRoutes.get('/rutinas', async (c) => {
  try {
    ...
    // Permitir simulación de fecha via query parameter
    const fechaSimulada = c.req.query('fecha_simulada')
    const fecha_hoy = fechaSimulada || new Date().toISOString().split('T')[0]
    console.log(`📅 Verificando rutinas para fecha: ${fecha_hoy}${fechaSimulada ? ' (SIMULADA)' : ''}`)
...
// Obtener preguntas específicas para una rutina
practicaRoutes.get('/rutinas/:id/preguntas', async (c) => {
  try {
    const rutinaId = c.req.param('id')
    
    const preguntas = await c.env.DB.prepare(`
    ...
    return c.json({
      success: true,
      data: preguntas.results
    })
  } catch (error) {
    return c.json({ success: false, error: 'Error al obtener preguntas de rutina' }, 500)
  }
})

// Completar rutina con respuestas detalladas
practicaRoutes.post('/rutinas/:id/completar-detallado', async (c) => {
  try {
    const rutinaId = c.req.param('id')
    const { 
      tiempo_invertido, 
    ...
    const fecha_hoy = new Date().toISOString().split('T')[0]
    const tiempo_inicio = new Date().toISOString()

    // Insertar o actualizar completada con datos detallados
    await c.env.DB.prepare(`
      INSERT OR REPLACE INTO rutinas_completadas 
    ...
    `).bind(
      ...
      estado_animo_antes || null,
      estado_animo_despues || null,
      calidad_percibida || null,
      ubicacion || null,
      tiempo_inicio,
...
// Obtener analytics de rutinas (últimos 30 días)
practicaRoutes.get('/rutinas/analytics', async (c) => {
  try {
    const { dias = 30 } = c.req.query()
    
    // Tendencias por rutina
    const tendenciasPorRutina = await c.env.DB.prepare(`
      SELECT 
        rm.nombre,
        rm.icono,
    ...
    // Progreso diario
    const progresoDiario = await c.env.DB.prepare(`
      SELECT 
        fecha,
        rutinas_completadas,
        rutinas_totales,
    ...
    return c.json({
      success: true,
      data: {
        tendencias_por_rutina: tendenciasPorRutina.results,
        progreso_diario: progresoDiario.results,
        racha_actual: rachaActual?.racha || 0,
        resumen: {
          dias_analizados: dias,
          fecha_inicio: new Date(Date.now() - dias * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
    ...
    return c.json({ success: false, error: 'Error al obtener analytics' }, 500)
  }
})
